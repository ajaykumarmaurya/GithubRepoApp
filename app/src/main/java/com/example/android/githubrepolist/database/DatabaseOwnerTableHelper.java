...
import com.example.android.githubrepolist.OwnerData;

import java.util.ArrayList;


public class DatabaseOwnerTableHelper {
	...
	/*private static final String COLUMN_KEY_BUSINESS_ADDRESS    		 = "address";

	private static final String COLUMN_KEY_BUSINESS_MOBILE    		 = "mobile";

	private static final String COLUMN_KEY_BUSINESS_CATEGORY    		 = "category";

	private static final String COLUMN_KEY_BUSINESS_WORKING_HOURS1    		 = "working_hours1";
	private static final String COLUMN_KEY_BUSINESS_WORKING_HOURS2    		 = "working_hours2";

	private static final String COLUMN_KEY_BUSINESS_WORKING_HOURS3    		 = "working_hours3";
	private static final String COLUMN_KEY_BUSINESS_WORKING_HOURS4    		 = "working_hours4";

	private static final String COLUMN_KEY_BUSINESS_WORKING_HOURS5    		 = "working_hours5";

	private static final String COLUMN_KEY_BUSINESS_WORKING_HOURS6    		 = "working_hours6";

	private static final String COLUMN_KEY_BUSINESS_WORKING_HOURS7    		 = "working_hours7";

	private static final String COLUMN_KEY_BUSINESS_KEYWORD   		 = "keywords";

	private static final String COLUMN_KEY_BUSINESS_FB_LINK    		 = "fb_link";
	private static final String COLUMN_KEY_BUSINESS_TWITTER_LINK      		 = "twitter_link";
	private static final String COLUMN_KEY_BUSINESS_INSTAGRAM_LINK      		 = "insta_link";

	private static final String COLUMN_KEY_BUSINESS_LINKEDIN_LINK      		 = "linkedin_link";
	*/
	...
	public DatabaseOwnerTableHelper(SQLiteDatabase database) {
		this.database = database;
	}

	public boolean insertRecord(OwnerData dataRecord) {
		...
		if ((String.valueOf(dataRecord.getOwner()) != null) &&
			(isRecordExists(dataRecord.getOwner()) == true)) {

			status = database.update(TABLE_NAME,
					values,
					COLUMN_KEY_OWNER + "=?" ,
					new String[] {dataRecord.getOwner()});
...
*/
	...
	public ArrayList<OwnerData> getAllDataRecord(Context context) {
		ArrayList<OwnerData> dataList = new ArrayList<OwnerData>();
		...
		if (c != null && c.moveToFirst()) {
			do {
				OwnerData dataCampaignRecord = getDataRecord(c, context);
	...
	/*public ArrayList<DataCampaignRecord> getAllPendingDataRecord(Context context) {
		ArrayList<DataCampaignRecord> dataList = new ArrayList<DataCampaignRecord>();
		
		Cursor c = database.query(TABLE_NAME,
				null,
				COLUMN_KEY_RECEIVE_STATUS + "=? OR " + COLUMN_KEY_CLICK_STATUS + "= ? OR " + COLUMN_KEY_CONVERT_STATUS + "= ?  OR " + COLUMN_KEY_DOWNLAOD_STATUS + "= ?",
				new String[] {String.valueOf(DataCampaignEventRecord.EVENT_STATUS_RECORDED), String.valueOf(DataCampaignEventRecord.EVENT_STATUS_RECORDED), String.valueOf(DataCampaignEventRecord.EVENT_STATUS_RECORDED), String.valueOf(DataCampaignEventRecord.EVENT_STATUS_RECORDED)},
				null,
				null,
				null);
		...
		if (c != null && c.moveToFirst()) {
			do {
				DataCampaignRecord dataCampaignRecord = getDataRecord(c, context);
				dataList.add(dataCampaignRecord);
			} while (c.moveToNext());
	...
	public OwnerData getDataRecordByMobile(String email,
			Context context) {
	...
	private OwnerData getDataRecord(Cursor c, Context context) {
		OwnerData dataRecord = new OwnerData();
	...
	private boolean isRecordExists(String owner) {
		boolean isCampaignExists = false;
		
		Cursor c = database.query(TABLE_NAME,
				 		new String[] {COLUMN_KEY_OWNER},
				COLUMN_KEY_OWNER + "=?",
				 		new String[] {owner},
				 		null,
				 		null,
